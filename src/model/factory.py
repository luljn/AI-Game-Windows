...
class Factory :
    ...
    def formFactory(self, window) :
        ...
        for element in forms :
            ...
            if element.__class__ == Square and element.getId() == 6 :
                
                circle = Circle(window, pygame.Vector2((window.getScreenWidth() / 2) - (square_.getWidth() + 10), (window.getScreenHeight() / 2) + (square_.getHeigth() + 10)), configs[1], element)
                forms.append(circle)
